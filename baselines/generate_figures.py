...
def get_parser():
    ...
    parser.add_argument(
        '-sequence-details-zurich',
        required=False,
        type=str,
        help='The participants.tsv file for Zurich containing MagneticFieldStrength.'
    )
    parser.add_argument(
        '-sequence-details-colorado',
        required=False,
        type=str,
        help='The participants.tsv file for Colorado containing MagneticFieldStrength.'
    )
    parser.add_argument(
        '-o',
        required=False,
    ...
    for metric in list_of_metrics:
        ...
        for method in df['method'].unique():
            ...
            for site in df['site'].unique():
                ...
                # If CSV files with MagneticFieldStrength are provided, loop across MagneticFieldStrength and stratify
                # the results for 1.5T and 3T
                if args.sequence_details_zurich and args.sequence_details_colorado:
                    # Loop across MagneticFieldStrength (1.5T, 3T)
                    for field in df_tmp['MagneticFieldStrength'].unique():
                        if math.isnan(field):
                            df_field = df_tmp[df_tmp['MagneticFieldStrength'].isna()]
                        else:
                            df_field = df_tmp[df_tmp['MagneticFieldStrength'] == field]
                        # NOTE: we cannot use 'len(df['participant_id'].unique())' because some subjects have multiple
                        # sessions
                        num_of_subjects = len(df_field[metric])
                        if pred_type == 'sc':
                            logger.info(f'\t{method} ({site}, {field}T, n={num_of_subjects}): '
                                        f'{df_field[metric].mean():.2f} ± {df_field[metric].std():.2f}')
                        elif pred_type == 'lesion':
                            logger.info(f'\t{method} ({site}, {field}T, n={num_of_subjects}): '
                                        f'{df_field[metric].mean():.2f} ± {df_field[metric].std():.2f}')
...
def main():
    ...
    if args.sequence_details_zurich and args.sequence_details_colorado:
        # Load MagneticFieldStrength for Colorado and Zurich from their respective participants.tsv files
        df_colorado = pd.read_csv(args.sequence_details_colorado, usecols=['participant_id', 'MagneticFieldStrength'],
                                  sep='\t')
        df_zurich = pd.read_csv(args.sequence_details_zurich, usecols=['participant_id', 'MagneticFieldStrength'],
                                sep='\t')
