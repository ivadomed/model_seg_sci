...
parser = argparse.ArgumentParser(description='Code for creating k-fold splits of the sci-zurich dataset.')

parser.add_argument('-se', '--seed', default=42, type=int, help="Seed for reproducibility")
parser.add_argument('-ncv', '--num_cv_folds', default=5, type=int, 
...
if args.num_cv_folds != 0:
    ...
    # create one json file with 85-15 train-test split
    ...
    for name, subs_list in train_subjects_dict.items():
        ...
        # calculate the median shapes along each axis
        params["train_median_shape"] = np.median(temp_shapes_list, axis=0).tolist()
    ...
    final_json = json.dumps(params, indent=4, sort_keys=True)
    jsonFile = open(root + "/" + f"dataset_0.json", "w")
    jsonFile.write(final_json)
    jsonFile.close()
