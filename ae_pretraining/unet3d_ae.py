...
class ModifiedUNet3DEncoder(nn.Module):
    ...
    def forward(self, x):
        ...
        out = self.norm_lrelu_upscale_conv_norm_lrelu_l0(out)   # shape: [batch-size x bnf*8 x 16 x 32 x 12]
...
class ModifiedUNet3D(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.cfg = cfg
        self.unet_encoder = ModifiedUNet3DEncoder(in_channels=1 if cfg.task == 'sc' else 2,
...
if __name__ == "__main__":
    x = torch.randn((2, 1, 128, 256, 96))
    unet_ae = ModifiedUNet3DEncoder(in_channels=1, base_n_filter=8, flatten=False)
    x_latent = unet_ae(x)
