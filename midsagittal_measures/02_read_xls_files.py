"""
Combine XLS files (one per subject) with lesion metrics generated by SCT's sct_analyze_lesion into a single CSV file
for easier manipulation and visualization.

Namely, the script:
 - reads XLS files (located under /results) with lesion metrics computed by SCT's sct_analyze_lesion
    (one XLS file per subject)
 - fetches the lesion metrics (such as lesion volume, etc.) from the XLS files
