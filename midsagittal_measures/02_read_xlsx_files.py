...
def get_parser():
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help='Branch name (e.g., master or PR4631) with the XLSX files with lesion metrics generated by '
    ...
    parser.add_argument(
        ...
        required=False,
        default='stats',
        help='Path to the output folder where XLSX table will be saved. Default: ./stats'
    )
...
def fetch_lesion_metrics(index, row, branch, df):
    ...
    # Check if the XLSX file with lesion metrics for manual lesion exists
    if not os.path.exists(row[branch]):
        raise ValueError(f'ERROR: {row[+branch]} does not exist.')

    # Read the XLSX file with lesion metrics for lesion predicted by our 3D SCIseg nnUNet model
...
def main():
    ...
    for index, row in df.iterrows():
        ...
        logger.info(f'Processing XLSX files for {row["participant_id"]}')

        # Read the XLSX file with lesion metrics
        df = fetch_lesion_metrics(index, row, branch_name, df)

    # remove the branch column containing the paths to the XLSX files
