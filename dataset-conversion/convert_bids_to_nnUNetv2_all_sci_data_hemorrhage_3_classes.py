...
REGION-BASED (https://github.com/MIC-DKFZ/nnUNet/blob/master/documentation/region_based_training.md):
    - 1 input channel: T2w image
    - multi-class labels:
        - `1` - spinal cord segmentation
        - `2` - hyperintense edema
        - `3` - hypointense hemorrhage
...
from utils import binarize_label, get_git_branch_and_commit, Image, create_multi_channel_label_input
...
LABEL_SUFFIXES = {
    "site_007": {"seg": "seg",
                 "lesion": "lesion",
                 "lesion_hemorrhage": "lesion_hemorrhage"}
}
...
def get_region_based_label(subject_hemorrhage_file, subject_image_file, site_name, sub_ses_name, thr=0.5):
    """
    Create region-based label for nnUNet training.
        0: background
        1: spinal cord seg
        2: lesion seg
        3: hemorrhage seg

    :param subject_hemorrhage_file: hemorrhage label file
    :param subject_image_file: T2w image file
    :param site_name:
    :param sub_ses_name:
    :param thr:
    :return: combined_label_file: region-based label file
    """
    # define path for sc seg file
    subject_seg_file = subject_hemorrhage_file.replace(f'_{LABEL_SUFFIXES[site_name]["lesion_hemorrhage"]}',
                                                       f'_{LABEL_SUFFIXES[site_name]["seg"]}')
    # define path for lesion (edema) seg file
    subject_lesion_file = subject_hemorrhage_file.replace(f'_{LABEL_SUFFIXES[site_name]["lesion_hemorrhage"]}',
                                                          f'_{LABEL_SUFFIXES[site_name]["lesion"]}')
    ...
    if not os.path.exists(subject_seg_file):
        ...
        return None
    if not os.path.exists(subject_lesion_file):
        logger.info(f"Lesion segmentation file for subject {sub_ses_name} does not exist. Skipping.")
        return None

    # load the labels
    seg_label_npy = nib.load(subject_seg_file).get_fdata()
    lesion_label_np = nib.load(subject_lesion_file).get_fdata()
    hemorrhage_label_npy = nib.load(subject_hemorrhage_file).get_fdata()

    # make sure the labels are binary
    seg_label_npy = np.where(seg_label_npy > thr, 1, 0)
    lesion_label_np = np.where(lesion_label_np > thr, 1, 0)
    hemorrhage_label_npy = np.where(hemorrhage_label_npy > thr, 1, 0)

    # add the lesion (edema) and hemorrhage seg to the spinal cord seg (because from the anatomical point of view,
    # the lesion is part of the spinal cord)
    seg_label_npy = np.where((seg_label_npy + lesion_label_np + hemorrhage_label_npy) > thr, 1, 0)

    # check if the shapes of the labels match
    assert hemorrhage_label_npy.shape == seg_label_npy.shape, \
    ...
    label_npy = np.zeros(hemorrhage_label_npy.shape, dtype=np.int16)
    # spinal cord
    label_npy[seg_label_npy == 1] = 1
    # lesion (edema) seg
    label_npy[lesion_label_np == 1] = 2
    # hemorrhage seg
    label_npy[hemorrhage_label_npy == 1] = 3

    # save the new label file
    ref = nib.load(subject_image_file)
    label_nii = nib.Nifti1Image(label_npy, ref.affine, ref.header)

    # save the region-based label
    combined_label_file = subject_hemorrhage_file.replace(f'_{LABEL_SUFFIXES[site_name]["lesion_hemorrhage"]}',
                                                          '_combined-label')
    nib.save(label_nii, combined_label_file)

    return combined_label_file
...
def create_dataset_json(args, train_ctr, test_ctr, dataset_commits, path_out):
    ...
    if args.region_based:
        ...
        json_dict['regions_class_order'] = [1, 2, 3]
    # MULTI-CHANNEL
...
def main():
    ...
    all_hemorrhage_files, train_images, test_images = [], {}, {}
    ...
    for dataset in args.path_data:
        ...
        # get recursively all GT 'lesion_hemorrhage' files
        hemorrhage_label_suffix = LABEL_SUFFIXES[site_name]["lesion_hemorrhage"]
        hemorrhage_files = [str(path) for path in root.rglob(f'*_{hemorrhage_label_suffix}.nii.gz')]
        # add to the list of all subjects
        all_hemorrhage_files.extend(hemorrhage_files)
    ...
    for subject_label_file in tqdm(all_hemorrhage_files, desc="Iterating over all images"):
        ...
        subject_image_file = (subject_label_file.replace('/derivatives/labels', '').
                              replace(f'_{LABEL_SUFFIXES[site_name]["lesion_hemorrhage"]}', ''))
        ...
        if subject_label_file in train_images.keys():
            ...
            # remove subject_label_file from the source dataset after copying
