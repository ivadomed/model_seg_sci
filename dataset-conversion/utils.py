...
def create_multi_channel_label(lesion_label_file, seg_label_file, image_file, sub_ses_name, thr=0.5):
    """
    Creates labels for MULTI-CHANNEL nnUNet training. The regions are:
    0: background
    1: lesion seg
    The function makes sure that the lesion seg is part of the spinal cord seg (the spinal cord seg is the first
    channel).
    """
    # load the labels
    lesion_label_npy = nib.load(lesion_label_file).get_fdata()
